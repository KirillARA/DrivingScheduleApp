...
[ApiController]
public class ЭкзаменController : ControllerBase
{
    private readonly DataService _dataService;
    public ЭкзаменController(DataService dataService) => _dataService = dataService;

    [HttpGet]
    public async Task<ActionResult<IEnumerable<ExamDto>>> GetAll()
        => Ok(await _dataService.GetAllExamsAsync());
